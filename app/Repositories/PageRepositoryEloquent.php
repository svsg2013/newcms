...
class PageRepositoryEloquent extends BaseRepository implements PageRepository
{
    ...
    public function getAllPageActive(){
        return $this->model->where('active', 1)->get(['id', 'code']);
    }

    public function search($keyword, $paginate = 0)
    {
        $result = $this->model->whereTranslationLike('title', '%'.$keyword.'%')->active();
