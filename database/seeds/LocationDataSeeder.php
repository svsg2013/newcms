...
class LocationDataSeeder extends Seeder
{
    ...
    public function run()
    {
        ...
        if (Schema::hasTable('city_career')
            && Schema::hasTable('city_career_translation')
            && \DB::table('city_career')->get()->count() < 1
        ) {
            \Eloquent::unguard();
            $path = 'database/seeds/city_career.sql';
            \DB::unprepared(file_get_contents($path));

            $this->command->info('City career table seeded!');
        }

        if (Schema::hasTable('districts')
            && Schema::hasTable('district_translations')
            && \DB::table('districts')->get()->count() < 1
        ) {
            \Eloquent::unguard();
            $path = 'database/seeds/districts.sql';
            \DB::unprepared(file_get_contents($path));

            $path = 'database/seeds/district_vi.sql';
            \DB::unprepared(file_get_contents($path));

            $path = 'database/seeds/district_en.sql';
            \DB::unprepared(file_get_contents($path));
        ...
        if (Schema::hasTable('wards')
            && Schema::hasTable('ward_translations')
            && \DB::table('wards')->get()->count() < 1
        ) {
            \Eloquent::unguard();
            $path = 'database/seeds/wards.sql';
            \DB::unprepared(file_get_contents($path));

            $path = 'database/seeds/ward_vi.sql';
            \DB::unprepared(file_get_contents($path));

            $path = 'database/seeds/ward_en.sql';
            \DB::unprepared(file_get_contents($path));
